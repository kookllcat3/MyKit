 ...
 * 2019.04.22      新增時依照ScrollView裡的陣列來判斷要顯示的Item
...
namespace Mosframe {
    ...
    public abstract class DynamicScrollView : UIBehaviour
    {
        ...
        /// <summary> ScrollView子物件更新時，要參考的資料陣列 </summary>
        ...
        protected override void Start ()
	    {
            ...
            this._prevTotalItemCount = this.dataList.Count;
            _itemSize += spacing;
	    ...
	    private void Update()
	    {
            if( this.dataList.Count != this._prevTotalItemCount ) {
        ...
        private void resizeContent() {
            ...
            if( this._direction == Direction.Vertical ) size.y = this._itemSize * this.dataList.Count - (dataList.Count > 0 ? spacing : 0);
            else                                        size.x = this._itemSize * this.dataList.Count - (dataList.Count > 0 ? spacing : 0);
	    ...
	    private void updateItem( int index, GameObject itemObj )
	    {
		    if( index < 0 || index >= this.dataList.Count ) {
                ...
                // 更新 Item時,將參數和移除item的方式都傳入.
			    var item = itemObj.GetComponent<IDynamicScrollViewItem>(); 
                var obj = (index >= DataList.Count) ? null : DataList[index];
                
                if ( item != null ) item.onUpdateItem( index, obj);
		    }
	    }
